...
export default function SentinelleVeillePage() {
    const [scannedSources, setScannedSources] = useState(0)

    useEffect(() => {
        const interval = setInterval(() => {
            setScannedSources(prev => (prev < 45 ? prev + 1 : 45))
        }, 100)
        ...
        <div className="p-8 space-y-8 bg-slate-950 min-h-screen text-slate-100">
                ...
                <div className="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-center gap-8">
                    <div className="space-y-4">
                        ...
                        <h1 className="text-4xl md:text-5xl font-black tracking-tighter">
                            Sentinelle <span className="text-amber-500">Jurisprudentielle</span>
                        </h1>
                        <p className="text-slate-400 text-lg font-light max-w-2xl">
                            Intelligence préventive : L'IA surveille les sources officielles et détecte les impacts sur vos dossiers en temps réel.
                        </p>
                    </div>

                    <div className="bg-white/5 border border-white/10 p-6 rounded-[2rem] w-full md:w-auto">
                        <div className="flex items-center justify-between mb-4 gap-8">
                            <p className="text-xs font-black uppercase text-slate-500 tracking-widest">Sources Surveillées</p>
                            <span className="text-2xl font-black text-amber-500">{scannedSources}</span>
                        </div>
                        <Progress value={(scannedSources / 45) * 100} className="h-2 bg-white/5 w-48" />
                        <div className="mt-4 flex gap-2 overflow-hidden italic text-[10px] text-slate-600 font-mono">
                            <span className="animate-pulse">CCJA...</span>
                            <span className="animate-pulse delay-75">JO SENEGAL...</span>
                            <span className="animate-pulse delay-150">COUR SUPREME...</span>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="space-y-8">
                    ...
                    <Button className="w-full h-14 bg-white text-slate-900 hover:bg-slate-100 font-black rounded-[1.5rem] gap-3">
                        <Gavel className="h-5 w-5" /> EXPÉDIEZ UN MÉMO À TOUTE L&apos;ÉQUIPE
                    </Button>
                </div>
