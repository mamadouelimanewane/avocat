...
const formatAndDownload = (doc: CrawledDoc) => {
    // Update status to loading/formatting
    setCrawledDocs(prev => prev.map(d => d.id === doc.id ? { ...d, status: 'formatted' } : d))
    alert(`Le document "${doc.title}" a été normalisé. Vous pouvez maintenant le valider pour la bibliothèque.`)
}
...
<div className="flex items-center gap-2">
    {doc.status === 'new' ? (
        <Button
            size="sm"
            variant="outline"
            className="border-slate-600 text-slate-400 hover:text-white hover:bg-slate-800 font-bold text-xs"
            onClick={() => formatAndDownload(doc)}
        >
            <FileText className="h-3 w-3 mr-2" /> FORMATER
        </Button>
    ) : doc.status === 'formatted' ? (
        <Button
            size="sm"
            className="bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-xs animate-in zoom-in"
            onClick={() => validateAndAdd(doc.id)}
        >
            <CheckCircle2 className="h-3 w-3 mr-2" /> VALIDER & CLASSER
        </Button>
    ) : (
        <Button size="sm" variant="outline" className="border-emerald-500/20 text-emerald-500 bg-emerald-500/5 cursor-default font-bold text-xs">
            <BookOpen className="h-3 w-3 mr-2" /> DANS BIBLIO
        </Button>
    )}
</div>
